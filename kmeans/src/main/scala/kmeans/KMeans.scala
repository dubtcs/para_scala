...
class KMeans extends KMeansInterface:
	...
	def classify(points: ParSeq[Point], means: ParSeq[Point]): ParMap[Point, ParSeq[Point]] =
		// Absolutely dreadful how much waste is here
		val pointsToClosest: ParMap[Point, Point] = points.map((p: Point) => (p -> findClosest(p, means))).toMap
		val clusters: ParMap[Point, ParSeq[Point]] = means.map(
			(m: Point) =>
				m -> pointsToClosest.filter((p: Point, m2: Point) => m2 == m).keys.toSeq.par
		).toMap
		clusters
	...
	def update(classified: ParMap[Point, ParSeq[Point]], oldMeans: ParSeq[Point]): ParSeq[Point] =
		val means: ParSeq[Point] = oldMeans.map(
			(mean: Point) =>
				if(classified.contains(mean))
					val p: Point = classified(mean).foldLeft(Point(0.0,0.0,0.0))(
						(prev: Point, cur: Point) =>
							Point(prev.x + cur.x, prev.y + cur.y, prev.z + cur.z)
					)
					val L: Int = classified(mean).length
					Point(p.x / L, p.y / L, p.z / L)
				else
					null
		)
		means
	...
	def converged(eta: Double, oldMeans: ParSeq[Point], newMeans: ParSeq[Point]): Boolean =
		val amount: Int = oldMeans.zipWithIndex.foldLeft(0)(
			(prev: Int, cur: (Point, Int)) =>
				if(cur._1.squareDistance(newMeans(cur._2)) <= eta)
					prev + 1
				else
					prev
		)
		amount == oldMeans.length

	@tailrec
	final def kMeans(points: ParSeq[Point], means: ParSeq[Point], eta: Double): ParSeq[Point] =
		val clusters: ParMap[Point, ParSeq[Point]] = classify(points, means)
		val newMeans: ParSeq[Point] = update(clusters, means)
