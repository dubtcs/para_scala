...
object LineOfSight extends LineOfSightInterface:
	...
	def upsweepSequential(input: Array[Float], from: Int, until: Int): Float =
		var r: Float = input.head;
		for {i <- (from + 1) until until} do
			r = r.max(input(i) / i) // need to div by height so it's the slope
		r
	...
	def upsweep(input: Array[Float], from: Int, end: Int, threshold: Int): Tree =
		if (end - from <= threshold)
			...
			val mid: Int = from + ((end - from) * 0.5).floor.toInt
			val nodes: (Tree, Tree) = parallel( upsweep(input, from, mid, threshold), upsweep(input, mid, end, threshold) )
			Tree.Node(nodes._1, nodes._2)
	...
	def downsweepSequential(input: Array[Float], output: Array[Float],
		...
		if(from < until)
			output(from) = startingAngle.max(input(from))
			for {i <- from + 1 until until} do
	...
	def downsweep(input: Array[Float], output: Array[Float], startingAngle: Float,
				  tree: Tree): Unit =
		tree match {
			case Tree.Leaf(from: Int, until: Int, n: Float) =>
				downsweepSequential(input, output, startingAngle, from, until)
			case Tree.Node(l: Tree, r: Tree) =>
				parallel(downsweep(input, output, startingAngle, l), downsweep(input, output, l.maxPrevious.max(startingAngle), r))
		}
